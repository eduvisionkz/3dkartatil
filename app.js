...
// Highlight the nav item for the section currently in view (only affects
// sections that actually exist on this page — cross-page nav items keep
// whichever "active" state was hard-coded in the page's HTML).
const navAnchors = [...document.querySelectorAll('[data-nav]')];
navAnchors.forEach(a => {
  const [hrefPage, hrefHash] = a.getAttribute('href').split('#');
  a.dataset.hrefPage = hrefPage || 'index.html';
  a.dataset.hrefHash = hrefHash || '';
});
const currentPage = (location.pathname.split('/').pop() || 'index.html');
const sectionIds = [...new Set(
  navAnchors.filter(a => a.dataset.hrefPage === currentPage && a.dataset.hrefHash)
            .map(a => a.dataset.hrefHash)
)];
  ...
  const navObserver = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        navAnchors.forEach(a => {
          const matches = a.dataset.hrefPage === currentPage && a.dataset.hrefHash === entry.target.id;
...
const mapAvailable = !!(oblastsLayer && regionsLayer && typeof KZ_MAP !== 'undefined' && typeof REGIONS !== 'undefined');
...
on('rotL','click', () => { rotZ -= 8; applyTransform(); });
on('rotR','click', () => { rotZ += 8; applyTransform(); });
on('zoomIn','click', () => { zoom = Math.min(1.6, zoom + 0.12); applyTransform(); });
on('zoomOut','click', () => { zoom = Math.max(0.7, zoom - 0.12); applyTransform(); });
on('resetMap','click', () => { rotZ = 0; zoom = 1; applyTransform(); selectedRegionId=null; document.querySelectorAll('.region-hot').forEach(g=>g.classList.remove('active')); renderPanel(null); });
let labelsOn = true;
on('toggleLabels','click', () => {
  labelsOn = !labelsOn;
...
(function initMapFullscreen(){
  ...
  const target = document.getElementById('mapFullscreenTarget');
  if (!btn || !target) return;

  function isActive(){
    return document.fullscreenElement === target || target.classList.contains('map-fullscreen-fallback');
  }
  function updateBtn(){
    const active = isActive();
    btn.textContent = active ? '✕ Толық экраннан шығу' : '⛶ Толық экран';
    btn.setAttribute('aria-pressed', active ? 'true' : 'false');
  }
  async function enter(){
    if (target.requestFullscreen) {
      try { await target.requestFullscreen(); return; } catch (e) { /* fall through to CSS fallback */ }
    }
    target.classList.add('map-fullscreen-fallback');
  ...
  async function exit(){
    ...
    target.classList.remove('map-fullscreen-fallback');
  ...
  document.addEventListener('keydown', e => {
    if (e.key === 'Escape' && target.classList.contains('map-fullscreen-fallback')) exit();
  });
  updateBtn();
})();
...
const periodsAvailable = !!(periodButtons && typeof PERIODS !== 'undefined');

function buildPeriods(){
  if (!periodsAvailable) return;
...
function updateIndicator(){
  if (!periodIndicator || !periodMini) return;
...
function setPeriod(id){
  activePeriod = id;
  if (periodButtons) periodButtons.querySelectorAll('.period-btn').forEach(b => b.classList.toggle('active', b.dataset.id === id));
  updateIndicator();
  renderPeriodContent();
  buildMap(); // refresh region opacity/relevance on map (no-op if map not on this page)
...
function renderPeriodContent(){
  ...
  periodContent.querySelectorAll('[data-jump]').forEach(el => {
    el.addEventListener('click', () => {
      // regions live on the map page — jump there with the region pre-selected
      sessionStorage.setItem('jumpToRegion', el.dataset.jump);
...
const compareAvailable = !!(comparePicker && compareTable && typeof REGIONS !== 'undefined');
let picked = compareAvailable ? [REGIONS[0].id, REGIONS[6].id] : [];
...
function renderStats(){
  ...
  if (!grid || typeof REAL_STATS === 'undefined') return;
...
function renderEthnicByRegion(){
  ...
  if (!rowsEl || typeof REAL_STATS === 'undefined') return;
  ...
  const ethnicLegend = document.getElementById('ethnicLegend');
  if (ethnicLegend) {
    ethnicLegend.innerHTML = [...usedKeys].map(k => {
...
function renderMethod(){
  const stepsEl = document.getElementById('methodSteps');
  const listEl = document.getElementById('methodList');
  if (stepsEl && typeof METHOD_STEPS !== 'undefined') {
    stepsEl.innerHTML = METHOD_STEPS.map((s,i) => `
      <div class="method-step reveal">
        <div class="num">${String(i+1).padStart(2,'0')}</div>
        <h4>${s.title}</h4>
        <p>${s.desc}</p>
      </div>`).join('');
    stepsEl.querySelectorAll('.reveal').forEach(el => revealObserver.observe(el));
  }
  if (listEl && typeof METHOD_LIST !== 'undefined') {
    listEl.innerHTML = METHOD_LIST.map(m => `<div>${m}</div>`).join('');
  }
}
...
function renderQuiz(){
  if (!quizCard || typeof QUIZ === 'undefined') return;
...
function renderAlphabetHistory(){
  ...
  if (!el || typeof ALPHABET_HISTORY === 'undefined') return;
...
// If we arrived on the map page via a "jump to region" link from another page,
// pre-select that region once the map has been built.
if (mapAvailable) {
  const jump = sessionStorage.getItem('jumpToRegion');
  if (jump) {
    sessionStorage.removeItem('jumpToRegion');
    selectRegion(jump);
  }
}
})();
